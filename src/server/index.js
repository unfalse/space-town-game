...
var getFileContents = function () { return JSON.parse(fs_1["default"].readFileSync(process.cwd() + "/src/server/" + filename).toString()); };
app.get("/list", function (request, response) {
    var contents = getFileContents();
    var names = contents.levels.map(function (level) { return ({ id: level.id, name: level.name }); });
    response.send(names);
});
app.get("/new", function (request) {
    ...
    fs_1["default"].writeFileSync(process.cwd() + "/src/server/" + "levelsTest.json", JSON.stringify(newContents));
});
app.get("/level", function (request, response) {
    var contents = getFileContents();
    var id = request.query.id;
    var level = contents.levels.find(function (level) { return level.id === id; });
    response.send(level);
});
app.post("/save", function (request, response) {
    ...
    fs_1["default"].writeFileSync(process.cwd() + "/src/server/" + filename, JSON.stringify(newContents));
